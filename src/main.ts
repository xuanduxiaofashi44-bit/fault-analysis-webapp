...
import { analyzeWorkbook, splitList, buildTypeSummary, buildLineTypeSummary, buildMonthSummary, getDeviceCount } from "./analysis";
import { renderCharts, resizeCharts, renderParetoInline, renderMttrInline, renderTrendInline, renderDailyTrendInline, renderMttrTrendInline, renderDailyMttrTrendInline, ensureChart } from "./charts";
...
let paretoMode: "total" | "byLine" = "total";
let searchText = "";
let monthFilter = "全部";
let typeFilter = "全部";
...
function syncConfigToUI(): void {
  ...
  const lineCountInput = document.querySelector<HTMLInputElement>("#lineCountInput");
  ...
  if (lineCountInput) lineCountInput.value = String(config.lineCount);
...
function bindEvents(): void {
  ...
  bindInputNoRun("#lineCountInput", (value) => { config.lineCount = Math.max(1, Number(value) || 4); });
  bindInputNoRun("#highlightInput", (value) => { config.highlightKeywords = splitList(value); });

  document.querySelector("#saveBasicBtn")?.addEventListener("click", () => { saveConfig(); recalcSummaries(); renderResult(); showToast("✓ 保存成功"); });
  ...
  document.querySelector("#addClassBtn")?.addEventListener("click", () => { config.classificationRules.push({ type: "新分类", field: "description", keywords: [] }); renderRuleEditors(); });
  ...
  document.querySelector("#paretoModeSelect")?.addEventListener("change", (event) => { paretoMode = (event.target as HTMLSelectElement).value as "total" | "byLine"; renderActiveChartOnly(); });
  ...
  document.querySelector("#lineFilter")?.addEventListener("change", (event) => { setLineFilter((event.target as HTMLSelectElement).value); });
  document.querySelector("#chartLineFilter")?.addEventListener("change", (event) => { setLineFilter((event.target as HTMLSelectElement).value); });
...
document.querySelector("#ownerFilter")?.addEventListener("change", (event) => { ownerFilter = (event.target as HTMLSelectElement).value; currentPage = 1; _invalidateFilterCache(); renderTable(); });
window.addEventListener("resize", resizeCharts);
  ...
  tableBody?.addEventListener("change", (event) => {
    ...
    if (target.classList.contains("inline-type-select")) {
      if (result) { const rec = result.records.find(r => r.id === (target as HTMLElement).dataset.id); if (rec) { rec.machineType = (target as HTMLSelectElement).value; _invalidateFilterCache(); renderTable(); } }
    } else if (target.classList.contains("row-check")) {
      const id = (target as HTMLInputElement).dataset.id ?? "";
      if ((target as HTMLInputElement).checked) { if (id) selectedIds.add(id); } else { if (id) selectedIds.delete(id); }
      renderTable();
    }
  });
}
...
function getFilteredRows(): FaultRecord[] {
  if (!result) return [];
const cacheKey = `${monthFilter}|${typeFilter}|${lineFilter}|${ownerFilter}|${searchText}|${sortKey}|${sortOrder}|${result.records.length}`;
...
.filter(r => ownerFilter === "全部" || r.owner === ownerFilter)
...
function getLineScopedResult(): AnalysisResult | null {
  if (!result) return null;
  const records = lineFilter === "全部" ? result.records : result.records.filter((record) => record.line === lineFilter);
  const months = [...new Set(records.map((record) => record.date.slice(0, 7)).filter(Boolean))].sort();
  return {
    ...result,
    records,
    months,
    typeSummary: buildTypeSummary(records),
    typeSummaryByMonth: Object.fromEntries(months.map((month) => [month, buildTypeSummary(records.filter((record) => record.date.startsWith(month)), month)])),
    monthSummary: buildMonthSummary(records, months, getDeviceCount(config))
  };
}

function setLineFilter(value: string): void {
  lineFilter = value;
  currentPage = 1;
  selectedIds.clear();
  _invalidateFilterCache();
  syncLineFilterSelects();
  updateLineScopedViews();
}

function syncLineFilterSelects(): void {
  document.querySelectorAll<HTMLSelectElement>("#lineFilter, #chartLineFilter").forEach((select) => {
    if (select.value !== lineFilter) select.value = lineFilter;
  });
}

function updateLineScopedViews(): void {
...
const classifyLocal = (record: FaultRecord): string => {
  ...
  r.records = r.records.map((record) => ({
    ...record,
    machineType: classifyLocal(record),
  ...
  r.monthSummary = buildMonthSummary(r.records, months, getDeviceCount(config));
...
function renderClassRules(): void {
  ...
  if (!container) return;
  container.innerHTML = config.classificationRules.map((rule, index) => `<div class="class-row"><input data-class-index="${index}" data-prop="type" value="${escapeHtml(rule.type)}" /><select data-class-index="${index}" data-prop="field"><option value="description" ${rule.field === "description" ? "selected" : ""}>问题描述</option><option value="machine" ${rule.field === "machine" ? "selected" : ""}>机器</option></select><textarea data-class-index="${index}" data-prop="keywords">${escapeHtml(rule.keywords.join(","))}</textarea><button data-class-index="${index}" data-prop="delete" class="icon-btn danger" title="删除">×</button></div>`).join("");
  container.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | HTMLButtonElement>("[data-class-index]").forEach((el) => { el.addEventListener("input", updateClassRule); el.addEventListener("click", updateClassRule); });
}

function updateClassRule(event: Event): void {
  const target = event.target as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | HTMLButtonElement;
...
if (prop === "field") config.classificationRules[index].field = (target as HTMLSelectElement).value as "machine" | "description";
...
const owners = [...new Set(result.records.map(r => r.owner).filter(Boolean))].sort(); ownerFilter = fillSelect("#ownerFilter", ["全部", ...owners], ownerFilter);
syncLineFilterSelects();
  _invalidateFilterCache();
...
function updateMetrics(scope: AnalysisResult): void {
  document.querySelector("#totalRecords")!.textContent = String(scope.records.length);
  document.querySelector("#totalDowntime")!.textContent = String(Math.round(scope.records.reduce((sum, r) => sum + r.downtime, 0)));
  document.querySelector("#totalTypes")!.textContent = String(scope.typeSummary.length);
  document.querySelector("#totalMonths")!.textContent = String(scope.months.length);
}
...
function renderTable(): void {
  ...
  tbodyNew.innerHTML = pageRows.length ? pageRows.map(r => `<tr class="${r.highlighted ? "highlight" : ""}"><td><input type="checkbox" class="row-check" data-id="${r.id}" ${selectedIds.has(r.id) ? "checked" : ""} /></td><td>${escapeHtml(r.date)}</td><td>${escapeHtml(r.line)}</td><td>${escapeHtml(r.startTime)}-${escapeHtml(r.endTime)}</td><td><span class="${downtimeClass(r.downtime)}">${r.downtime}</span></td><td>${r.machineType === "未分类" ? `<select class="inline-type-select" data-id="${r.id}"><option selected>未分类</option>${config.classificationRules.map(cr => `<option>${escapeHtml(cr.type)}</option>`).join("")}</select>` : escapeHtml(r.machineType)}</td><td>${escapeHtml(r.description)}</td><td>${escapeHtml(r.department)} / ${escapeHtml(r.owner)}</td></tr>`).join("") : `<tr><td colspan="8" class="empty">暂无符合条件的数据</td></tr>`;
...
function renderActiveChartOnly(): void {
  const chartResult = getLineScopedResult();
  if (!chartResult) return;
  const typeData = selectedMonth === "合计" ? chartResult.typeSummary : chartResult.typeSummaryByMonth[selectedMonth] ?? [];
  ...
  } else if (activeChart === "mttrTrend") {
    const mt = ensureChart("mttrTrend");
    if (mt) {
      if (selectedMonth === "合计") renderMttrTrendInline(mt, chartResult);
      else renderDailyMttrTrendInline(mt, chartResult, selectedMonth, getDeviceCount(config));
    }
  } else {
    const t = ensureChart("trend");
    if (t) {
      if (selectedMonth === "合计") renderTrendInline(t, chartResult);
      else renderDailyTrendInline(t, chartResult, selectedMonth, getDeviceCount(config));
    }
  }
